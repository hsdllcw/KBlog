...
public class ZipUtil {
    ...
    private static File buildFile(File outFile, String fileName) {
        if (!FileUtil.isWindows() && StrUtil.contains(fileName, CharUtil.SLASH)) {
            // 在Linux下多层目录创建存在问题，/会被当成文件名的一部分，此处做处理
            // 使用/拆分路径（zip中无\），级联创建父目录
//            final String[] pathParts = StrUtil.splitToArray(fileName, CharUtil.SLASH);
            final String[] pathParts = Arrays.stream(StrUtil.splitToArray(fileName, CharUtil.SLASH)).filter(StringUtils::isNotBlank).toArray(String[]::new);
