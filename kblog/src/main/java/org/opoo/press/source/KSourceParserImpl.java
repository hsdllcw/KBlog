...
public class KSourceParserImpl extends SourceParserImpl implements SourceParser {
    ...
    public Source parse(SourceEntry sourceEntry) throws NoFrontMatterException {
        ...
        try {
            ...
            if (isFrontMatterStartLine(iterator.next(), sourceEntry)) {
                return super.parse(sourceEntry);
            }
            IOUtils.writeLines(IteratorUtils.toList(iterator), null, contentWriter);
        } catch (IOException e) {
        ...
        try {
            ...
            Page page = pageService.findByPath(sourceEntry.getFile().getPath().substring(pageService.getBasedir().getPath().length()));
            if (page != null) {
